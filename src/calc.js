...
export function simulate(input) {
  ...
  const lastsFrom = (portfolioAtStop, y, perpetualFloor, suppliedBalances = null) => {
    ...
    if (fiYear >= planEndYear || reserveCash(balances, retirementReserve) > 1e-6) return false;
    ...
    if (portfolioTotal(balances) - retirementReserve < floorIn(perpetualFloor, Math.floor(fiYear), fiYear, y) - 1e-6) return false;
    for (let year = Math.floor(fiYear); year < planEndYear; year++) {
      const investedFor = year + 1 - Math.max(year, fiYear);
      const step = retirementStep(balances, Math.max(0, needIn(year, fiYear) - pensionIncomeIn(year, y)),
        investedFor, a.cashRealReturn ?? 0, a.realReturn, retirementReserve);
  ...
  const resilience = () => {
    ...
    const survives = (badYears, badReturn, crash = 0) => {
      ...
      if (portfolioTotal(balances) - retirementReserve < floorIn(perpetualMode, Math.floor(fiYear), fiYear, yearsToFi) - 1e-6) {
        return false;
      }
      for (let i = 0; i < draws.length; i++) {
        const step = retirementStep(balances, draws[i], durations[i], a.cashRealReturn ?? 0,
          i < badYears ? badReturn : a.realReturn, retirementReserve);
        if (step.shortfall > 1e-6) return false;
        balances = step.balances;
  ...
  if (Number.isFinite(yearsToFi) && (countPots || countState) &&
    ...
    for (let year = Math.floor(currentYear + yearsToFi); year < planEndYear; year++) {
      ...
      balances = retirementStep(balances, Math.max(0, needIn(year, currentYear + yearsToFi) - income),
        investedFor, a.cashRealReturn ?? 0, a.realReturn, retirementReserve).balances;
    }
  ...
  if (Number.isFinite(yearsToFi)) {
    ...
    for (let year = Math.floor(currentYear + yearsToFi); year < planEndYear; year++) {
      ...
      const investedFor = year + 1 - Math.max(year, currentYear + yearsToFi);
      const step = retirementStep(balances, fromPortfolio, investedFor,
        a.cashRealReturn ?? 0, a.realReturn, retirementReserve);
      balances = step.balances;
      schedule.push({
        year,
