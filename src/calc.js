...
// Chronological quarterly/event search: late feasibility need not imply global
// monotonicity. Narrow unsampled windows can still be missed. Local bisection
// retains a tested feasible upper bound, but does not prove a unique crossing.
export function firstFundedDate(ok, horizon, events = []) {
  if (!Number.isFinite(horizon) || horizon <= 0) return Infinity;
  if (ok(0)) return 0;
  const checkpoints = new Set([horizon, ...events]);
  for (let y = .25; y < horizon; y += .25) checkpoints.add(y);
  let lo = 0;
  for (const y of [...checkpoints].filter((y) => Number.isFinite(y) && y > 0 && y <= horizon)
    .sort((a, b) => a - b)) {
    if (!ok(y)) { lo = y; continue; }
    let hi = y;
    for (let i = 0; i < 50; i++) {
      const mid = (lo + hi) / 2;
  ...
  const solveFor = (perpetualFloor) => {
    ...
    for (const p of people) {
      for (const date of [p.pillar2DrawYear, p.pillar3DrawYear,
        p.statePensionYear, p.pillarIncomeEndYear, healthEndFor(p)]) {
        if (date != null) events.push(date - currentYear);
      }
    }
    return firstFundedDate(ok, horizon, events);
  };
