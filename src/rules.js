...
export function actionPlan(sim, input) {
  ...
  // Age and employment only identify a route to investigate. They do not prove
  // marriage/registered partnership, dependency, insured status or approval.
  const someoneEmployed = people.some((p) => p.employed);
  for (const p of people) {
    const yearsToPension = p.pension.statePensionAge - p.ageNow;
    const possibleSpouseRoute = someoneEmployed && !p.employed &&
      yearsToPension <= RATES.healthInsurance.dependentSpouseNearPensionYears &&
      yearsToPension >= 0;
    if (possibleSpouseRoute && !p.healthInsurance) {
      add({
        id: 'dependent-spouse-cover', severity: 'opportunity', link: 'health-insurance',
        title: `${p.name || 'This person'}: check dependent-partner coverage`,
        value: 'Eligibility not confirmed',
        detail:
          `A near-pension-age dependent spouse or registered partner may qualify through an ` +
          `insured supporting partner. These inputs do not establish legal partnership, ` +
          `dependency, the applicable age boundary or the supporting person's insured status. ` +
          `Apply to Tervisekassa and confirm the dates before removing any insurance budget. ` +
          `Stopping work is not necessarily the date coverage ends. This possible route does ` +
          `not suppress the unconfirmed-coverage warning or change the projection.`,
      });
    }
    ...
    if (!insured) {
      add({
        ...
        detail:
          `The plan does not confirm current health coverage. Check employment, family, ` +
          `state-pension and other eligible routes with Tervisekassa. Eligible registered ` +
          `unemployed people can have insurance without receiving unemployment benefit; ` +
          `confirm registration conditions and coverage dates with Töötukassa. ` +
          `If no other route applies, the published voluntary premium is ` +
          `${eur(RATES.healthInsurance.voluntaryMonthly)}/month. The contract lasts one year, ` +
          `normally starts after one month and has specified termination conditions; signing ` +
          `while existing cover is valid can preserve continuity. Co-payments remain. ` +
          `Temporary coverage is not a confirmed lifelong coverage start.`,
      });
    }
  }
